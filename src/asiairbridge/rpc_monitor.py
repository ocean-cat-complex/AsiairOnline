...
RPC_MONITOR_HTML = r"""<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>ASIAIR 动态信息监控</title>
  <style>
    :root {
      color-scheme: light;
      --bg: #f5f6f8;
    ...
    * { box-sizing: border-box; }
    ...
    .chip.ok { color: var(--ok); background: var(--ok-soft); border-color: #bdebd3; }
    .chip.fail { color: var(--fail); background: var(--fail-soft); border-color: #ffd0cc; }
    .chip.pending { color: var(--pending); background: var(--pending-soft); border-color: #f3dda2; }
    .metrics {
      display: grid;
...
<body>
  <header class="top">
    <div class="top-inner">
      ...
      <div class="controls">
        ...
        <button id="refresh" class="primary" type="button">刷新详情</button>
  ...
  <script>
    ...
    function statusBadge(item) {
      if (item.pending) return `<span class="badge pending">等待</span>`;
      if (item.ok) return `<span class="badge ok">成功</span>`;
      return `<span class="badge fail">未成功</span>`;
    }
    ...
    function render(data) {
      state.data = data;
      if (!state.device) state.device = data.device.name;
      renderDevices(data.devices, data.device.name);
      renderStatus(data);
      renderMetrics(data);
      renderSide(data.categories);
      renderContent(data.categories);
    }

    function renderDevices(devices, active) {
      const select = $("device");
      const options = devices.map((device) => (
        `<option value="${device.name}" ${device.name === active ? "selected" : ""}>${device.name} · ${device.ip}</option>`
      )).join("");
    ...
    function renderStatus(data) {
      ...
      const chips = [
        `<span class="chip ${data.refreshing ? "pending" : "ok"}">${data.refreshing ? "后台全局刷新中" : "后台刷新就绪"}</span>`,
        `<span class="chip">设备 ${data.device.name} · ${data.device.ip}</span>`,
        `<span class="chip">更新 ${data.generated_at}</span>`,
        `<span class="chip">核心状态 ${summary.ready_count || 0} 项</span>`,
        `<span class="chip ok">成功 ${summary.ok_count || 0}</span>`,
        `<span class="chip fail">未成功 ${summary.fail_count || 0}</span>`,
      ];
      $("status-line").innerHTML = chips.join("");
      renderRefreshProgress(data.refresh_progress);
    }

    function renderRefreshProgress(progress) {
      const box = $("refresh-progress");
      const button = $("refresh");
      if (!progress || !progress.total) {
        box.classList.add("hidden");
        button.disabled = false;
        button.textContent = "刷新详情";
        return;
      }
      ...
      button.textContent = active ? "刷新详情中" : "刷新详情";
      box.classList.remove("hidden");
      box.innerHTML = `
        <div class="progress-copy">
          <strong>${active ? "详情刷新中" : "详情刷新完成"}</strong>
          <span>已尝试 ${done} 项 · 成功 ${fmt(progress.ok, 0)} · 未成功 ${fmt(progress.fail, 0)}${progress.current ? ` · ${escapeHtml(progress.current)}` : ""}</span>
        </div>
        <div>${percent}%</div>
    ...
    function renderMetrics(data) {
      ...
      const storage = h.storage_free_mb && h.storage_total_mb
        ? `${Math.round(h.storage_free_mb / 1024)} / ${Math.round(h.storage_total_mb / 1024)} GB`
        : "—";
      $("metrics").innerHTML = [
        metric("当前页面", h.page),
        metric("拍摄状态", h.capture_working ? `进行中 · ${fmt(h.capture_state)}` : fmt(h.capture_state)),
        metric("目标", h.target_name),
        metric("目标进度", progressText(h.target_progress)),
        metric("序列 / 帧", `${fmt(h.sequence_type)} · ${progressText(h.frame_progress)}`),
        metric("相机", `${fmt(h.camera_name)} · ${fmt(h.camera_state)}`),
        metric("曝光 / Bin", `${fmt(h.exposure_seconds)}s · Bin ${fmt(h.bin)}`),
        metric("温度 / 制冷", `${fmt(h.temperature_c)}°C · ${fmt(h.cooler_power_percent)}%`),
        metric("存储剩余", storage),
      ].join("");
    }

    function renderSide(categories) {
        if (!state.activeCategory && categories.length) state.activeCategory = categories[0].id;
      $("side").innerHTML = categories.map((cat) => (
        `<button type="button" data-cat="${cat.id}" class="${cat.id === state.activeCategory ? "active" : ""}">
          <span>${cat.label}</span><span>${cat.ready_count} 项</span>
        </button>`
      )).join("");
      $("side").querySelectorAll("button").forEach((btn) => {
        btn.addEventListener("click", () => {
          state.activeCategory = btn.dataset.cat;
          renderSide(state.data.categories);
    ...
    function renderContent(categories) {
      ...
      if (!categories.length) {
        content.innerHTML = `<div class="empty">暂无监控数据。</div>`;
        return;
      }
      content.innerHTML = categories.map((cat) => renderCategory(cat)).join("");
      content.querySelectorAll(".section").forEach((section) => {
        section.style.display = section.dataset.cat === state.activeCategory ? "" : "none";
      });
    }

    function renderCategory(cat) {
      const items = cat.items.map(renderItem).join("");
      return `<section class="section" data-cat="${cat.id}">
        <div class="section-head">
          <div>
            <h2>${cat.label}</h2>
            <p>已查询 ${cat.ready_count} 项，${cat.ok_count} 成功，${cat.fail_count} 未成功</p>
          </div>
          <div class="group-stats">
            <span class="chip ok">成功 ${cat.ok_count}</span>
            <span class="chip fail">未成功 ${cat.fail_count}</span>
          </div>
        </div>
    ...
    function renderDisplayFields(item) {
      const rows = Array.isArray(item.display_rows) ? item.display_rows : [];
      const rowHtml = rows.length
        ? `<div class="field-grid">${rows.map((row) => `
            <div class="field-row">
              <span class="field-label">${escapeHtml(fmt(row.label, ""))}</span>
              <span class="field-value">${escapeHtml(fmt(row.value, ""))}</span>
            </div>
          `).join("")}</div>`
        : "";
      const tableHtml = renderDisplayTable(item.display_table);
      return rowHtml + tableHtml;
    }
    ...
    function renderDisplayTable(table) {
      ...
      const head = `<thead><tr>${table.columns.map((col) => `<th>${escapeHtml(fmt(col, ""))}</th>`).join("")}</tr></thead>`;
      const body = `<tbody>${table.rows.map((row) => (
        `<tr>${table.columns.map((_, index) => `<td>${escapeHtml(fmt(row[index], ""))}</td>`).join("")}</tr>`
      )).join("")}</tbody>`;
      return `<table class="mini-table">${caption}${head}${body}</table>`;
    }

    function renderItem(item) {
      const params = item.params === null || item.params === undefined ? "无" : JSON.stringify(item.params);
      const code = item.code === null || item.code === undefined ? "—" : item.code;
      const result = item.pending ? "等待后台刷新" : (item.result !== undefined ? JSON.stringify(item.result, null, 2) : item.error || "");
      const key = `${item.port}|${item.method}|${params}`;
      const open = state.openDetails.has(key) ? "open" : "";
      const updated = item.updated_at ? item.updated_at.split("T").pop() : "等待首次查询";
      const error = item.error ? `<span>错误 ${item.error}</span>` : "";
      const wide = item.display_wide ? " wide" : "";
      return `<article class="item${wide}">
        <div class="item-head">
        ...
        <div class="live-value">${escapeHtml(fmt(item.display_value))}</div>
        <div class="live-detail">${escapeHtml(fmt(item.display_detail, ""))}</div>
        ${renderDisplayFields(item)}
        <div class="meta">
          <span>更新 ${updated}</span>
          ${error}
        </div>
        <details data-key="${escapeHtml(key)}" ${open}>
          <summary>原始返回值与调用信息</summary>
          <div class="tech">
            <code>${item.method}</code>
            <code>${params}</code>
            <span>端口 ${item.port}</span>
            <span>code ${code}</span>
            <span>耗时 ${fmt(item.seconds)}s</span>
            <span>后台间隔 ${item.interval_seconds}s</span>
          </div>
    ...
    async function load(force = false) {
      if (state.paused || state.requestInFlight) return;
      ...
      try {
        ...
        const res = await fetch(`/api/rpc-monitor?${params.toString()}`);
        const data = await res.json();
        if (!res.ok || !data.ok) throw new Error(data.error || `HTTP ${res.status}`);
        render(data);
        document.querySelectorAll("details[data-key]").forEach((detail) => {
          detail.addEventListener("toggle", () => {
            if (detail.open) state.openDetails.add(detail.dataset.key);
            else state.openDetails.delete(detail.dataset.key);
          });
        });
      } catch (err) {
        $("status-line").innerHTML = `<span class="chip fail">轮询失败：${escapeHtml(err.message)}</span>`;
      } finally {
    ...
    load(false);
    state.timer = setInterval(() => load(false), 1000);
  </script>
</body>
</html>
"""
